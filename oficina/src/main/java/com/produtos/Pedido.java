	...
	public boolean incluirItem(ItemDePedido novo) {
		...
		for(int i=0; i<quantProdutos; i++) {
			if(itens[i].equals(novo)) {
				...
				System.out.println("Produto já inserido. Deseja alterar a quantidade no carrinho? (1)sim (0)nao");
				int resp = teclado.nextInt();
				if(resp == 1)
					itens[i].setQuantidade(itens[i].getQuantidade() + novo.getQuantidade());
				
				return true;
			}
		}
		
		//se nao existe no carrinho
		if (quantProdutos < MAX_PRODUTOS) {
			if(novo.estoqueDisponivel())
